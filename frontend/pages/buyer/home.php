...
layout('header', ['title' => 'Discover']);
?>
<?php component('flash'); ?>

<!-- Hero -->
<div class="card" style="background: linear-gradient(120deg, var(--color-primary), #ff8a65); color:#fff; margin-bottom:24px;">
    <h2 style="font-size:24px;">Hi <?= e($user['name']) ?> 👋</h2>
    <p style="opacity:.95;">Find quality products from rated &amp; trusted sellers.</p>
    <div class="row mt-2">
        <a class="btn" style="background:rgba(255,255,255,.18); color:#fff;" href="#categories">Browse categories</a>
        <a class="btn" style="background:#fff; color:var(--color-primary);" href="<?= base_url('/frontend/pages/buyer/wishlist.php') ?>">❤️ My wishlist</a>
    </div>
</div>
...
<!-- Categories -->
<h3 id="categories" class="mb-2">Categories</h3>
<div class="row mb-3" style="gap:8px; flex-wrap:wrap;">
    <a class="tag <?= empty($filters['category_id']) ? 'tag-primary' : '' ?>" href="<?= base_url('/frontend/pages/buyer/home.php') ?>">All</a>
...
<div class="card card-tight mb-3" style="display:flex; gap:8px; align-items:center; flex-wrap:wrap;">
    <span class="text-muted fs-13">Sort:</span>
    <?php
    $sorts = [
        '' => 'Newest',
        'popular' => 'Popular',
