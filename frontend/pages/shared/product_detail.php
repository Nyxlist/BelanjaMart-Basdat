...
$wished  = $user['role'] === 'buyer' ? WishlistModel::has((int) $user['user_id'], $id) : false;
...
<div class="grid" style="grid-template-columns: 1.4fr 1fr; gap: 24px;">
    <div class="card center" style="background:var(--bg-muted); min-height:340px; font-size:80px;">
        <?= e($p['category_icon'] ?? '📦') ?>
    </div>
    <div class="card">
        <span class="tag tag-primary"><?= e($p['category_icon'] ?? '📦') ?> <?= e($p['category_name']) ?></span>
        ...
        <div class="row mt-2" style="gap:8px;">
            <?php if ($user['role'] === 'buyer' && $p['stock'] > 0): ?>
                ...
                <form method="POST" action="<?= base_url('/frontend/pages/buyer/wishlist.php') ?>" style="display:inline;">
                    ...
                    <button class="btn btn-outline btn-lg"><?= $wished ? '❤️ Wishlisted' : '🤍 Wishlist' ?></button>
                </form>
...
<div class="card mt-3">
    ...
    <div class="row" style="gap:16px; flex-wrap:wrap;">
        ...
        <div style="flex:1; min-width:240px;">
            <div class="fw-bold fs-18"><?= e($p['shop_name'] ?? $p['seller_name']) ?> <?php if (!empty($p['seller_verified'])): ?>✅<?php endif; ?></div>
